...
Two pipeline implementations exist, each producing vector indexes from different input sources and using different processing strategies:

```mermaid
graph TD
    subgraph "Plaintext pipeline (active)"
...
|----------|------------|---------|-----------|--------|
| **Plaintext** | `scripts/generate_embeddings.py` | LlamaIndex FAISS | `lightspeed-rag-tool-push/pull-request` | 3.12 |
...
```mermaid
graph TD
    subgraph "Containerfile (plaintext pipeline)"
        B_BASE[UBI9 Python 3.12 CPU] --> B_DEPS[pip install requirements]
        B_DEPS --> B_COPY[Copy docs + runbooks + model]
        B_COPY --> B_LOOP["For each OCP version:<br/>python3.12 generate_embeddings.py"]
        B_LOOP --> B_LINK[Create latest symlink]
        B_LINK --> B_FINAL[ubi-minimal final image]
    end

    B_FINAL --> IMG["/rag/vector_db/<br/>/rag/embeddings_model/"]
```

The build follows a two-stage pattern: a builder stage generates all vector indexes (one per OCP version), then a minimal final stage copies only the output artifacts. The final image contains the indexes at `/rag/vector_db/ocp_product_docs/{version}/` and the embedding model at `/rag/embeddings_model/`.
...
- **model.safetensors** reassembled from committed `embeddings_model/model.safetensors.tar.gz.*` chunks
...
├── Containerfile                 # RAG content image
...
- **Two pipeline implementations exist.** The plaintext pipeline (LlamaIndex FAISS) is the active CI path. The HTML pipeline is for development. The former lsc library pipeline (llamastack-faiss) has been retired; OCP docs are now served by an OKP sidecar.
...
- **CPU-only build.** The GPU flavor has been retired. A single set of CPU lockfiles is used for all builds.
