...
# EC rejects Hermeto's binary-package SBOM attribute for these PyPI packages.
NO_WHEEL_PACKAGES="markupsafe packaging"
...
cleanup() {
	rm -f "$RAW_REQ_FILE" "$WHEEL_FILE" "$SOURCE_FILE" "$SOURCE_FILE_FOR_BUILD" "$BUILD_FILE_TMP"
}
...
# Generate a complete build-dependency lockfile. A stale lockfile can omit the
# PEP 517 backend required by a newly added source distribution, so fail instead
# of reporting a successful regeneration when pybuild-deps cannot resolve it.
BUILD_FILE_TMP=$(mktemp)
if uv run --no-project --with "pybuild-deps==0.5.0" pybuild-deps compile --output-file="$BUILD_FILE_TMP" "$SOURCE_FILE_FOR_BUILD"; then
	mv "$BUILD_FILE_TMP" "$BUILD_FILE"
else
	rm -f "$BUILD_FILE_TMP"
	echo "ERROR: pybuild-deps failed; $BUILD_FILE was left unchanged" >&2
	exit 1
fi
